...
public class OpenAIProvider extends BaseAIProvider {
    ...
    public OpenAIProvider(PluginSettings settings) {
        super(settings);
    }
    ...
    public String generateCommitMessage(@NotNull String diffContent, @NotNull List<FileChangeInfo> fileChanges)
        ...
        try {
            ...
            if (responseCode == 200) {
                ...
                throw new AIProviderException("OpenAI API error (" + responseCode + "): " + response.toString());
            }

        } catch (AIProviderException e) {
            throw e;
        } catch (Exception e) {
            LOG.warn("OpenAI provider error", e);
